...
def save(config, model, index, acc, save_type):
    assert save_type in ['model', 'optim', 'scheduler']
    ...
    if (save_type == 'optim'):
        ...
        torch.save(model.state_dict(), save_path+'/{}_scheduler_params_epoch_{:08d}_acc_{}.pth'.format(config['trainer']['model']['name'], index, acc))
...
def get_confusion_matrix(actual, predictions):
    cm = confusion_matrix(actual, predictions)
    return [
        # actual_left_pred_left = cm[0, 0],
        # actual_left_pred_right = cm[0, 0],
        # actual_left_pred_front = cm[0, 0],
        cm[0, 0],
        cm[0, 1],
        cm[0, 2],
        cm[1, 0],
...
def get_classification_report(actual, predictions):
    report = classification_report(actual, predictions, target_names=['left', 'right', 'front'], output_dict=True)
    return report
