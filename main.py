...
if __name__ == "__main__":
        ...
        if (config_dict['transformer']['enable_benchmark_test'] == True):
            transform(
                config_dict['transformer'][
                    'test_path'], config_dict['transformer']['fps'],
                config_dict['transformer']['test_save_file'], [
                    config_dict['data']['HEIGHT'], config_dict['data']['WIDTH']],
                config_dict['data']['CHANNELS'])

    df_videos = dict(np.load(config_dict['transformer'][
                     'data_save_file'] + '_video.npz', allow_pickle=True))
    print(df_videos.keys())

    if (config_dict['transformer']['enable_benchmark_test'] == True):
        test_videos = dict(np.load(config_dict['transformer'][
                           'test_save_file'] + '_video.npz', allow_pickle=True))
    ...
    train_transforms = va.Sequential([
    ...
    # following functions returns a list of file paths (relative paths to
    # video csvs) for train and test sets
    ...
    # else:
    train_files, val_files = make_tt_split(
        list(df_videos.keys()), config_dict['global']['seed'])

    print("Train Files:", train_files)
    print("Val Files:", val_files)

    trainer = Trainer(config_dict, train_transforms, val_transforms,
                      train_files, val_files, df_videos, df_sensor, test_videos, test_sensor)
    trainer.save(0, -1)
    ...
    for epoch in range(epochs):
        ...
        display_classification_report(
            train_actual, train_predictions, val_actual, val_predictions)
