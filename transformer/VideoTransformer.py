...
class VideoTransformer(object):
    ...
    def __init__(self, fps=10, resolution=[512, 512], channels=3):
        ...
        self.va_transforms = va.Sequential([
            va.RandomCrop(size=(240, 180)),
            va.RandomRotate(degrees=10),
        ])

    def _getVideo(self, filename):
        video_stream = self.va_transforms(ffmpeg.input(filename))
        return video_stream
    ...
    def _getFrames(self, videoCapture, fps, ref_time=-1, secs_in_past=-1, secs_in_future=-1):
        ...
        if ref_time != -1 and secs_in_past != -1 and secs_in_future != -1:
            videoCapture = videoCapture.trim(
                start=ref_time - secs_in_past, duration=secs_in_past + secs_in_future).filter('setpts', 'PTS-STARTPTS')

        videoCapture = videoCapture.filter('fps', fps=fps, round='up').filter(
            'scale', w=self.resolution[0], h=self.resolution[1])
        return videoCapture
    ...
    def _convertToNumpy(self, frames):
        ...
        video = np.frombuffer(out, np.uint8).reshape(
            [-1, self.resolution[0], self.resolution[1], self.channels])[1:, :, :, :]
        return video

    def transform(self, filename, ref_time=-1, secs_in_past=-1, secs_in_future=-1):
        video = self._getVideo(filename)
        frames = self._getFrames(
            video, self.fps, ref_time, secs_in_past, secs_in_future)
    ...
    def scrape_all_data(self, path):
        ...
        for directory in directories:
            # to ignore the temp files that get created like ".DS_Store"
            if directory.startswith("."):
                continue
            dir_path = os.path.join(path, directory)
            video_files = [f for f in os.listdir(
                dir_path) if f.endswith('.mp4')]
            print(video_files)
